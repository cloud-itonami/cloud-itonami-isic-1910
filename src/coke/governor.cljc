(ns coke.governor
  "Coke Oven Operations Governor -- the independent compliance layer that earns
  the Coke Operations Advisor the right to propose and log actions.
  The LLM has no notion of coke-plant safety standards, emissions regulations,
  or when a production-run scheduling or suspension is a real-world actuation,
  ...
  HARD violations (a human approver CANNOT override):
    ...
    4. Process-control operations -- NO furnace/process-control decisions
  ...
  coke-oven process. It does NOT:
    - Control furnace charge/discharge
    - Control coking process temperature, timing, or parameters
    - Operate emissions-system hardware (monitors only)
    - Make process-engineering decisions about coal quality/composition

  Those remain the exclusive authority of licensed plant engineers."
  (:require [coke.store :as store]))

(def confidence-floor 0.6)

(def high-stakes
  "Operations that require human sign-off for real-world actuation:
  Production-run scheduling and emissions reporting with exceedances."
  #{:actuation/schedule-production-run :actuation/log-emissions-report})
...
(def process-control-keywords
  ...
  #{"furnace-control" "charge" "discharge" "temperature-setpoint" "coking-time"
    "pressure-regulation" "process-parameters" "combustion-control"
    "furnace" "temperature" "setpoint" "parameters"})
...
(defn- spec-basis-violations
  ...
  [proposal _st]
  (let [op (:op proposal)]
    (when (contains? #{:actuation/schedule-production-run
...
(defn- process-control-block-violations
  ...
  If a proposal mentions furnace control, temperature setpoints, charging,
  or other process parameters, reject it immediately.
  Those decisions remain the exclusive authority of licensed plant engineers."
  [proposal _st]
...
(defn- supplier-verification-violations
  "Raw material intake requires verified supplier license."
  [{:keys [op subject]} st]
  (when (= op :proposal/intake-raw-material)
    (when-not (store/supplier-verified? st subject)
      [{:rule :supplier-not-verified
        :detail "サプライヤーのライセンスが未確認"}])))

(defn- material-verification-violations
  "Production-run scheduling requires verified coal batch."
  [{:keys [op subject]} st]
  (when (= op :actuation/schedule-production-run)
    (let [run (store/production-run st subject)
          coal-batch (:coal-batch run)]
      (when-not (store/material-verified? st coal-batch)
        [{:rule :material-not-verified
          :detail "石炭バッチが未検証"}]))))
...
(defn evaluate
  ...
  (let [hard-checks-store [spec-basis-violations
                           ...
                           material-verification-violations]
